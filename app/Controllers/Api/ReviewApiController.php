...
class ReviewApiController extends AuthController {
    ...
    public function index(array $params): void {
        ...
        $stmt = $pdo->prepare('SELECT cr.id,cr.rating,cr.review AS comment,cr.created_at,u.first_name,u.last_name FROM course_reviews cr JOIN courses c ON c.id=cr.course_id JOIN users u ON u.id=cr.user_id WHERE c.uuid=? AND cr.is_approved=1 ORDER BY cr.created_at DESC LIMIT 50');
    ...
    public function store(array $params): void {
        ...
        $pdo->prepare('INSERT INTO course_reviews (course_id,user_id,rating,review) VALUES (?,?,?,?)')->execute([$c['id'],$user['id'],$rating,$comment]);
