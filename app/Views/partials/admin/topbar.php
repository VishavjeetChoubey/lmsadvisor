...
<header class="admin-topbar">
  ...
  <!-- ── Desktop-only tools bar ──────────────────────────────────────────── -->
  <div class="d-none d-lg-flex align-items-center gap-2 me-2" id="adminToolsBar" style="font-size:12px">

    <!-- 1. Learner View button -->
    <a href="<?= $url('learn/dashboard') ?>" target="_blank"
      ...
      <i class="bi bi-mortarboard-fill" style="font-size:13px"></i>
      Learner View
    </a>

    <div style="width:1px;height:20px;background:var(--border-color);opacity:.4"></div>

    <!-- 2. Page load speed -->
    <div title="Page load time" style="display:inline-flex;align-items:center;gap:5px;color:var(--text-muted);cursor:default">
      <i class="bi bi-lightning-charge-fill" style="font-size:13px;color:#f59e0b"></i>
      <span id="adminLoadTime" style="font-weight:600;color:var(--text-primary)">—</span>
    </div>

    <!-- 3. Internet speed / connectivity -->
    <div id="adminNetWrap" title="Network status" style="display:inline-flex;align-items:center;gap:5px;cursor:default">
      <i class="bi bi-wifi" id="adminWifiIcon" style="font-size:15px;color:#9ca3af"></i>
      <span id="adminNetLabel" style="font-weight:600;color:var(--text-muted);font-size:11.5px">—</span>
    </div>

    <!-- 4. Local time -->
    <div title="Your local time" style="display:inline-flex;align-items:center;gap:5px;color:var(--text-muted)">
      <i class="bi bi-clock-fill" style="font-size:13px;color:#6366f1"></i>
      <span id="adminLocalTime" style="font-weight:600;color:var(--text-primary);font-variant-numeric:tabular-nums">—</span>
    </div>
...
<!-- ── Desktop tools JS (SOC2: no external APIs, all browser-native) ─────── -->
<script>
(function() {
  // ── 1. Page load time ──────────────────────────────────────────────────────
  // Uses browser Navigation Timing API — no external calls, no PII
  window.addEventListener('load', function() {
    var el = document.getElementById('adminLoadTime');
  ...
  // ── 2. Network status & speed ──────────────────────────────────────────────
  // Uses browser Network Information API — no external calls, no PII
  function updateNet() {
    var icon  = document.getElementById('adminWifiIcon');
    var label = document.getElementById('adminNetLabel');
    ...
    var speed = conn ? conn.downlink : null; // Mbps, browser estimate

    var color, text;
    if (speed === null) {
      color = '#6366f1'; text = 'Online';
    } else if (speed >= 5) {
      color = '#059669'; text = speed.toFixed(0) + ' Mbps';
    } else if (speed >= 1) {
      color = '#d97706'; text = speed.toFixed(1) + ' Mbps';
    } else {
      color = '#dc2626'; text = 'Slow';
    }
  ...
  // ── 3. Local time clock ────────────────────────────────────────────────────
  // Uses browser Date — no external calls, no PII
  function updateClock() {
    var el = document.getElementById('adminLocalTime');
    if (!el) return;
    var now = new Date();
